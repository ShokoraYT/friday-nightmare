...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		oogie.updateHitbox();
		oogie.setGraphicSize(Std.int(oogie.width * 0.7));
		oogie.scrollFactor.set(0, 0);
		...
		cutie.updateHitbox();
		cutie.setGraphicSize(Std.int(cutie.width * 0.7));
		cutie.scrollFactor.set(0, 0);
		cutie.antialiasing = ClientPrefs.globalAntialiasing;
		add(cutie);
		...
		{
			var offset:Float = 178 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(940, (i * 100)  + offset);
			menuItem.frames = Paths.getSparrowAtlas('mainmenu/menu_' + optionShit[i]);
			menuItem.animation.addByPrefix('idle', optionShit[i] + "Idle", 24);
			menuItem.animation.addByPrefix('selected', optionShit[i] + "ButtonHOVER", 24);
			menuItem.animation.addByPrefix('confirm', optionShit[i] + "Confirm", 24);
		...
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 64, 0, "Friday Nightmare", 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
